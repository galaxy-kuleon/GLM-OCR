...
10. **Do NOT insert CJK/Latin spaces** — never insert new whitespace between a CJK character and an adjacent Latin letter, digit, or ASCII punctuation mark unless the source text already contains that exact whitespace. Example: `使用OpenAI API`, not `使用 OpenAI API`.
```
...
- Each batch contains approximately **30 segments** (reduced from 50 to prevent LLM output truncation with smaller models)
...
- **Each batch result is written to a separate file** (`batch_N.json`) for crash recovery
- After all batches, a completeness validation step verifies 100% coverage
...
| ~30 segments per batch | Balances context length, translation quality, and throughput |
