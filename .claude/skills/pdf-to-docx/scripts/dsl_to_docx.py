...
def apply_run_attrs(run, elem, page_font_latin, page_font_cjk):
    ...
    if parse_bool(elem.get("subscript")):
        run.font.subscript = True
    if parse_bool(elem.get("strikethrough")):
        rPr = run._r.get_or_add_rPr()
        strike = OxmlElement("w:strike")
        rPr.append(strike)

    highlight = elem.get("highlight-color")
    if highlight:
        set_run_shading(run, highlight)
...
def process_runs(para, elem, page_font_latin, page_font_cjk):
    ...
    if not runs:
        ...
        text = _sanitize_text((elem.text or "").strip())
        if text:
    ...
    for run_elem in runs:
        text = _sanitize_text(run_elem.text or "")
        if not text:
            continue
        run = para.add_run(text)
...
def process_heading(doc, elem, page_font_latin, page_font_cjk):
    ...
    runs = elem.findall("run")
    if runs:
        heading_text = _sanitize_text("".join((r.text or "") for r in runs))
    else:
        heading_text = _sanitize_text((elem.text or "").strip())
    ...
    if runs:
        for i, run_elem in enumerate(runs):
            if i < len(heading.runs):
                ...
                if parse_bool(run_elem.get("underline")):
                    doc_run.font.underline = True
                if parse_bool(run_elem.get("superscript")):
                    doc_run.font.superscript = True
                if parse_bool(run_elem.get("subscript")):
                    doc_run.font.subscript = True
                if parse_bool(run_elem.get("strikethrough")):
                    _rPr = doc_run._r.get_or_add_rPr()
                    _strike = OxmlElement("w:strike")
                    _rPr.append(_strike)
                h_highlight = run_elem.get("highlight-color")
                if h_highlight:
                    set_run_shading(doc_run, h_highlight)
...
def process_paragraph(doc, elem, page_font_latin, page_font_cjk):
    ...
    # Indentation
    indent_left = elem.get("indent-left-cm")
    indent_right = elem.get("indent-right-cm")
    indent_first = elem.get("indent-first-line-cm")
    if indent_left:
        para.paragraph_format.left_indent = Cm(float(indent_left))
    if indent_right:
        para.paragraph_format.right_indent = Cm(float(indent_right))
    if indent_first:
        para.paragraph_format.first_line_indent = Cm(float(indent_first))

    # List nesting via indentation (marker text is preserved in content)
    list_level = elem.get("list-level")
    if list_level and not indent_left:
        para.paragraph_format.left_indent = Cm(0.63 * int(list_level))
...
def _infer_col_widths_from_content(elem, num_cols):
    """Infer column width ratios from cell content lengths.

    When VLM produces uniform col-widths (all equal), use the maximum
    text length per column as a heuristic to compute better ratios.
    Returns a list of ratios summing to ~1.0, or None if inference fails.
    """
    col_max_len = [1] * num_cols  # min 1 to avoid division by zero
    for row_elem in elem.findall("row"):
        for cell_elem in row_elem.findall("cell"):
            col_attr = cell_elem.get("col")
            if col_attr is None:
                continue
            col_idx = int(col_attr)
            colspan = int(cell_elem.get("colspan", "1"))
            if colspan > 1 or col_idx >= num_cols:
                continue
            # Get text length
            run_children = cell_elem.findall("run")
            if run_children:
                text = "".join((r.text or "") for r in run_children)
            else:
    ...
    if total == 0:
        return None
    # Apply sqrt to dampen extreme ratios (a cell with 100 chars shouldn't be
    # 10x wider than one with 10 chars)
    dampened = [max(0.08, x ** 0.5) for x in col_max_len]
    total_d = sum(dampened)
    return [round(x / total_d, 3) for x in dampened]


def _col_widths_are_uniform(ratios):
    """Check if column width ratios are suspiciously uniform (all equal)."""
    if not ratios or len(ratios) < 2:
        return False
    first = ratios[0]
    return all(abs(r - first) < 0.02 for r in ratios)
...
def process_table(doc, elem, page_font_latin, page_font_cjk, workspace):
    ...
    # If VLM generated uniform col-widths, try to infer better ones from content
    if col_ratios and _col_widths_are_uniform(col_ratios):
        inferred = _infer_col_widths_from_content(elem, num_cols)
        if inferred:
            col_ratios = inferred
    ...
    for row_elem in elem.findall("row"):
        ...
        for cell_elem in row_elem.findall("cell"):
            ...
            run_children = cell_elem.findall("run")
            if run_children:
                cell_text = _sanitize_text("".join((r.text or "") for r in run_children))
            else:
                cell_text = _sanitize_text((cell_elem.text or "").strip())
            ...
            if run_children:
                ...
                for run_elem in run_children:
                    ...
                    if parse_bool(run_elem.get("strikethrough")):
                        _rPr = docx_run._r.get_or_add_rPr()
                        _strike = OxmlElement("w:strike")
                        _rPr.append(_strike)
                    r_highlight = run_elem.get("highlight-color")
                    if r_highlight:
                        set_run_shading(docx_run, r_highlight)
            else:
...
def _resolve_image_path(workspace, src):
    """Resolve an image src path with multiple fallback strategies.

    Tries: exact path, ocr-output prefix, filename-only in known dirs,
    and glob-based fuzzy matching for off-by-one index errors.

    Returns Path if found, None otherwise.
    """
    ws = Path(workspace)

    # Strategy 1: exact path relative to workspace
    candidate = ws / src
    if candidate.exists():
        return candidate

    # Strategy 2: try with ocr-output/input prefix
    candidate = ws / "ocr-output" / "input" / src
    if candidate.exists():
        return candidate

    # Strategy 3: filename only in known image directories
    filename = Path(src).name
    for img_dir in [
        ws / "ocr-output" / "input" / "imgs",
        ws / "input-images",
        ws / "imgs",
    ]:
        if img_dir.is_dir():
    ...
    # Strategy 4: glob-based fuzzy match for off-by-one index errors
    # e.g. cropped_page4_idx0.jpg doesn't exist but cropped_page4_idx1.jpg does
    import re as _re
    match = _re.match(r"(cropped_page\d+_idx)(\d+)(\.jpg)", filename)
    if match:
        prefix, idx_str, suffix = match.groups()
        idx = int(idx_str)
        imgs_dir = ws / "ocr-output" / "input" / "imgs"
        if imgs_dir.is_dir():
            # Try nearby indices (idx-1, idx+1, idx+2)
            for delta in [-1, 1, 2]:
                alt = imgs_dir / f"{prefix}{idx + delta}{suffix}"
                if alt.exists():
                    return alt
            # Last resort: glob for any image with matching page prefix
            page_prefix = prefix.split("_idx")[0]
            candidates = sorted(imgs_dir.glob(f"{page_prefix}_idx*{suffix}"))
            if candidates:
                # Return the first available one (best effort)
                return candidates[0]
...
def process_image(doc, elem, workspace, page_font_latin, page_font_cjk):
    ...
    # Resolve image path with robust fallback
    img_path = _resolve_image_path(workspace, src)
    if img_path is None:
...
def process_text_frame(doc, elem, page_font_latin, page_font_cjk):
    # Support both bbox (normalized 0-1000, from VLM) and direct twips (legacy)
    bbox_str = elem.get("bbox")
    page_width_pts = float(elem.get("page-width-pts", "595"))
    page_height_pts = float(elem.get("page-height-pts", "842"))

    if bbox_str:
        # Convert bbox (normalized 0-1000) to twips (1 pt = 20 twips)
        parts = [float(x) for x in bbox_str.split(",")]
        if len(parts) == 4:
            x1, y1, x2, y2 = parts
            x_twips = int(x1 / 1000.0 * page_width_pts * 20)
            y_twips = int(y1 / 1000.0 * page_height_pts * 20)
            w_twips = int((x2 - x1) / 1000.0 * page_width_pts * 20)
            h_twips = int((y2 - y1) / 1000.0 * page_height_pts * 20)
        else:
            x_twips, y_twips, w_twips, h_twips = 0, 0, 2000, 1000
    else:
        # Legacy: direct twips values
...
def process_horizontal_rule(doc, elem):
    """Add a horizontal rule as a paragraph with a bottom border."""
    para = doc.add_paragraph()
    color = elem.get("color", "auto") if elem is not None else "auto"
    size = elem.get("size", "6") if elem is not None else "6"
...
def process_page_header(doc, elem, page_font_latin, page_font_cjk):
    """Add header content to the current section."""
    section = doc.sections[-1]
    header = section.header
    header.is_linked_to_previous = False

    first_para = header.paragraphs[0] if header.paragraphs else None
    if first_para:
        first_para.clear()
    ...
    if paragraphs:
        for i, p_elem in enumerate(paragraphs):
            para = first_para if (i == 0 and first_para) else header.add_paragraph()
            alignment = get_alignment(p_elem.get("alignment"))
            if alignment is not None:
                para.alignment = alignment
            process_runs(para, p_elem, page_font_latin, page_font_cjk)
        ...
        text = (elem.text or "").strip()
        if text and first_para:
            run = first_para.add_run(text)
            run.font.name = page_font_latin
            run.font.size = Pt(9)


def process_page_footer(doc, elem, page_font_latin, page_font_cjk):
    """Add footer content to the current section."""
    section = doc.sections[-1]
    footer = section.footer
    footer.is_linked_to_previous = False

    first_para = footer.paragraphs[0] if footer.paragraphs else None
    if first_para:
        first_para.clear()
    ...
    if paragraphs:
        for i, p_elem in enumerate(paragraphs):
            para = first_para if (i == 0 and first_para) else footer.add_paragraph()
            alignment = get_alignment(p_elem.get("alignment"))
            if alignment is not None:
                para.alignment = alignment
            process_runs(para, p_elem, page_font_latin, page_font_cjk)
...
def process_page(doc, page_xml_path, workspace, is_first_page):
    ...
    # Process child elements in order (skip comments)
    for child in root:
        if callable(child.tag):
            continue  # XML comment node
        tag = child.tag
        if tag == "heading":
        ...
        elif tag == "horizontal-rule":
            process_horizontal_rule(doc, child)
        elif tag == "page-header":
            process_page_header(doc, child, page_font_latin, page_font_cjk)
        elif tag == "page-footer":
            process_page_footer(doc, child, page_font_latin, page_font_cjk)
        elif tag == "col-widths":
...
# ---------------------------------------------------------------------------
# Header/footer auto-detection from pdf-fulltext.txt
# ---------------------------------------------------------------------------


def detect_header_footer_from_fulltext(workspace):
    """Detect repeating header/footer lines from pdf-fulltext.txt.

    Scans for lines that appear on most pages (identified by "Page N" markers).
    Returns (header_text, footer_text) or (None, None).
    """
    fulltext_path = Path(workspace) / "pdf-fulltext.txt"
    ...
    # Split by page markers (e.g. "Page 1", "Page 2" at line boundaries)
    page_pattern = re.compile(r"^Page\s+\d+\s*$", re.MULTILINE)
    page_breaks = list(page_pattern.finditer(text))
    ...
    for i, match in enumerate(page_breaks):
        # Lines before this "Page N" marker are footer candidates
        before_text = text[:match.start()].rstrip()
        if i > 0:
            prev_end = page_breaks[i - 1].end()
            before_text = text[prev_end:match.start()].rstrip()

        before_lines = [l.strip() for l in before_text.split("\n") if l.strip()]
        for line in before_lines[-2:]:
            if len(line) > 5 and not re.match(r"^Page\s+\d+$", line):
                footer_candidates[line] = footer_candidates.get(line, 0) + 1

    # Check first few lines of each page section for header candidates
    # Also check lines immediately after the "Page N" marker
    for i in range(len(page_breaks)):
        if i + 1 < len(page_breaks):
            section = text[page_breaks[i].end():page_breaks[i + 1].start()]
        else:
            section = text[page_breaks[i].end():]
    ...
    footer_text = None
    for line, count in sorted(footer_candidates.items(), key=lambda x: -x[1]):
        if count >= threshold:
            footer_text = line
            break

    header_text = None
    for line, count in sorted(header_candidates.items(), key=lambda x: -x[1]):
        if count >= threshold:
            header_text = line
            break
...
def apply_auto_header_footer(doc, header_text, footer_text):
    """Apply auto-detected header/footer text to the DOCX document.

    Only applies if no header/footer was already set by DSL processing.
    """
    ...
    if header_text:
        header = section.header
        header.is_linked_to_previous = False
        existing = "".join(p.text for p in header.paragraphs).strip()
        if not existing:
...
def main():
    ...
    # Auto-detect and apply header/footer if none was set by DSL
    header_text, footer_text = detect_header_footer_from_fulltext(workspace)
    if header_text or footer_text:
        print(f"Auto-detected header: {header_text!r}")
        print(f"Auto-detected footer: {footer_text!r}")
        apply_auto_header_footer(doc, header_text, footer_text)
