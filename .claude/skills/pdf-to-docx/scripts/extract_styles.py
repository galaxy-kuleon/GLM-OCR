#!/usr/bin/env python3
"""extract_styles.py - Extract text styling from PDF page images using LMStudio API.

Usage:
    python extract_styles.py --workspace PATH --pages N [--api-key KEY]

For each page, calls LMStudio (qwen3.5-122b-a10b) with a simplified prompt to
...
API key is read from --api-key argument or LMSTUDIO_API_KEY environment variable
or .env file in workspace. LMStudio typically does not require a real key.
"""
...
def load_api_key(args_key, workspace):
    """Load API key from args, env, or .env file. LMStudio does not require a real key."""
    if args_key:
        return args_key
    if os.environ.get("LMSTUDIO_API_KEY"):
        return os.environ["LMSTUDIO_API_KEY"]
    ...
    if cwd_env.exists():
        for line in cwd_env.read_text().splitlines():
            ...
            if line.startswith("LMSTUDIO_API_KEY="):
                return line.split("=", 1)[1].strip().strip("\"'")
    # LMStudio does not require a real API key; return a dummy key
    return "lm-studio"
...
def call_api_table_cell_styles(api_key, page_image_b64, num_rows, num_cols):
    """Call LMStudio API to extract cell-level color overrides from a table image.
...
def call_api(api_key, page_image_b64, regions_summary):
    """Call LMStudio API and return parsed JSON array."""
    if requests is None:
        raise RuntimeError("requests library not available")
...
def main():
    parser = argparse.ArgumentParser(description="Extract text styles using LMStudio API")
    ...
    parser.add_argument(
        "--api-key", default=None, help="LMStudio API key (or set LMSTUDIO_API_KEY env)"
    )
    ...
    # Load API key
    api_key = load_api_key(args.api_key, workspace)
    api_available = api_key is not None and requests is not None

    if not api_available:
        if not api_key:
            print(
                "Warning: LMSTUDIO_API_KEY not found. Using default styles only.",
                file=sys.stderr,
            )
        if requests is None:
            print(
