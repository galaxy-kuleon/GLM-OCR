#!/usr/bin/env python3
"""review_dsl.py - Review XML DSL against PDF page images using LMStudio API.

Usage:
    python review_dsl.py --workspace PATH --pages N [--api-key KEY]
...
or .env file. LMStudio typically does not require a real key.
"""
...
def call_api_review(api_key, page_image_b64, xml_content):
    """Call LMStudio API for single-page DSL review and return parsed JSON array."""
    if requests is None:
...
def call_api_review_multi(api_key, page_images_b64, xml_contents, page_numbers):
    """Call LMStudio API for multi-page DSL review.
...
def main():
    parser = argparse.ArgumentParser(
        description="Review XML DSL against page images using LMStudio API"
    )
    ...
    if not api_key:
        print("Warning: LMSTUDIO_API_KEY not found. Skipping VLM review.", file=sys.stderr)
        # Write empty reviews
