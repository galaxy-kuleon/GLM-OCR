...
Usage:
    python verify_docx_visual.py --workspace PATH --pages N --docx PATH [--api-key KEY]
...
def call_api_visual_compare(api_key, content_parts, system_prompt, timeout=300):
    """Call LMStudio API for visual comparison."""
    if requests is None:
...
def main():
    ...
    parser.add_argument("--api-key", default=None, help="LMStudio API key")
    ...
    if not api_key:
        print("WARNING: LMSTUDIO_API_KEY not found. Skipping visual verification.", file=sys.stderr)
        for pn in range(1, input_pages + 1):
            out_path = dsl_dir / f"visual-review-page-{pn}.json"
            out_path.write_text("[]")
